...
import { btcRpcUrl, btcFields, amfeixFeeFields, data } from '../core/data';
...
class Ethereum_P2P_Network extends Comp {
  componentDidMount() { amfeixAddressLists.concat(ethFields).concat(["ethRPCUrl"]).forEach(f => this.addSyncKeyObserver(data, f)); }
  ren(p, s) {
    return tabulize(2/3, [
      [<List data={[{ name: "RPC url", value: s.ethRPCUrl }].concat(ethFields.map(name => ({ name, value: S(oO(s[name])) })))} />],
